...
class LoginWindow(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.setGeometry(100, 100, 400, 250)  # Increased height to accommodate the "Show Password" button
        ...
        try:
            eye_icon = QtGui.QPixmap(gui_setup.image1_path).scaled(30, 30, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
            eye_off_icon = QtGui.QPixmap(gui_setup.image2_path).scaled(30, 30, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
    ...
    def toggle_password_visibility(self):
        if self.show_password_button.isChecked():
            ...
            try:
                eye_off_icon = QtGui.QPixmap(gui_setup.image2_path).scaled(270, 340, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
                self.show_password_button.setIcon(QtGui.QIcon(eye_off_icon))  # Change icon to eye-off
            except FileNotFoundError:
                print("Error: Image file not found. Make sure hide_password.png is in the correct directory.")
        else:
            self.password.setEchoMode(QtWidgets.QLineEdit.Password)
            try:
                eye_icon = QtGui.QPixmap(gui_setup.image1_path).scaled(270, 340, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
                self.show_password_button.setIcon(QtGui.QIcon(eye_icon))  # Change icon to eye
            except FileNotFoundError:
                print("Error: Image file not found. Make sure show_password.png is in the correct directory.")
    ...
    def check_login(self):
        ...
        if username == "admin" and password == "admin":
            self.message_label.setText('Logging in...')
            self.message_label.setStyleSheet("font-size: 14px; color: green;")
        else:
            self.message_label.setText('Incorrect password or username')
            self.message_label.setStyleSheet("font-size: 14px; color: red;")
